...
├── helpdesk\_project/     # Project settings, urls, custom exception handler
...
\- `ticket` (FK), `change\_description`, `created\_at`
...
3\. Status can only move forward in sequence — `OPEN → IN\_PROGRESS → RESOLVED → CLOSED`.
...
Full request/response examples are in `helpdesk\_api\_handoff.md` (shared with the
...
&#x20;   "field\_name": \["error message here"]
...
`django-cors-headers` is enabled with `CORS\_ALLOW\_ALL\_ORIGINS = True` for local
